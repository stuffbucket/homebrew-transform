...
class FormulaProcessor
  ...
  # Required attributes for a valid formula (brew audit --strict)
  REQUIRED_ATTRIBUTES = %w[desc homepage].freeze
  # Must have either version or url (url implies version from download)
  VERSION_ATTRIBUTES = %w[version url].freeze
  ...
  def validate_formula(content, path)
    missing = REQUIRED_ATTRIBUTES.reject { |attr| content.match?(/\b#{attr}\s*[(\s]/) }
    missing.each { |attr| warn "Warning: #{path} may be missing required attribute '#{attr}'" }

    has_version = VERSION_ATTRIBUTES.any? { |attr| content.match?(/\b#{attr}\s*[(\s]/) }
    warn "Warning: #{path} may be missing 'version' or 'url'" unless has_version
  end
